...
-  Alternatively, if only one power supply is available, then P4 & P5 jumpers
   can be left in place and a single field supply used.

.. warning::

   If either P4 and/or P5 jumpers are inserted, then there will be no isolation
   between the system domain and the field power domain.
...
.. note::
   ...
   Resistor values between 0 Ω and 1 kΩ can be used. However, lower values
   provide a more challenging load and demonstrate the DPC mode power savings
   better.
...
.. note::
   ...
   (466 mW X (1-η)). Which translates to 513 mW of dissipation per channel in
   the module.
...
Notice how VDPC+ is now at its minimum value of approximately 4.95 V and the die
temperature begins to drop rapidly as DPC power saving take effect.
   \ |image9|\ 

...
Launch the PC application, connect to the ezLINX board, then Press the Reset
button SW2 for the first time from the ezLINX board, as is shown in the
following figure a message is displayed to inform that the factory reset is done
by keep pressing the button (SW2) more than 20 seconds.

.. image:: https://wiki.analog.com/_media/resources/eval/ezlinx/v1.1.3/kernelrestoreguide/factoryresetinformationmessage.jpg
   :align: center
   :width: 850

After Keep pressing the SW2 button on the ezLINX board for 20 seconds, the Leds
are flashing and a sequence of messages will be displayed in order to inform
about the remaining time to the factory reset also to inform user that he should
not remove the power. Exemples of message which are displayed : The first is to
inform that 80 seconds are remained for factory Rest process.
...
The third, as it is shown in the figure below, is to tell that the factory reset
is going to last 60 seconds.

.. image:: https://wiki.analog.com/_media/resources/eval/ezlinx/v1.1.3/kernelrestoreguide/thirdwarningmessage.jpg
   :align: center
   :width: 850

The fourth message is displayed in the following figure to inform that 50
seconds are remained.
...
Without using the PC application, Keep pressing the SW2 button on the ezLINX
board for 20 seconds, until the Leds start flashing.
...
In the drop down menu next to "Connect using:" select "COM1" and click "OK". In
the following properties menu select the following values using the drop down
menus:
...
After a brief period of time the board will reconnect with the PC. The kernel
has been restored to its default settings and the IP address of the board has
returned to its default (192.168.1.21).

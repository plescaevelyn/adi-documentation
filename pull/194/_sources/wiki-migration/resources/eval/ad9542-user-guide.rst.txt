AD9542 Evaluation Board User Guide
==================================
...
-  AD9542 ACE Plug-in Module: `board.ad9542.1.1.8.zip <https://wiki.analog.com/_media/resources/eval/board.ad9542.1.1.8.zip>`_
-  Please only use ACE Software version 1.10.2671.1118. Do not update it to newer versions as they are not tested with the AD9542 plug-in module
-  Install ACE first. Unzip the plug-in. Double click on it to have it installed

Documents Needed

-  :adi:`AD9542 Datasheet <media/en/technical-documentation/data-sheets/AD9542.pdf>`
-  :adi:`AD9542 Register Map User's Guide (UG-1166) <media/en/technical-documentation/user-guides/AD9542-Register-Map-Reference-Manual-UG-1166.pdf>`
-  `ACE User Guide <http://swdownloads.analog.com/ACE/ACE_User_Manual_rev3.pdf>`_

GENERAL DESCRIPTION
-------------------

The :adi:`AD9542` evaluation board is a compact, easy-to-use platform for evaluating all features of the :adi:`AD9542` dual digital PLL, synchronizer, and jitter cleaner. The :adi:`AD9542` provides high-precision, multi-output clock generator functions, along with two on-chip jitter cleaning digital PLL cores. PLL0 and PLL1 are optimized for high performance synchronous clocking applications such as Synchronous Ethernet, OTN, and next generation wireless baseband protocols. The PLLs are fully configurable via serial port control as well as configurable via an external EEPROM for power on ready configurations.

The :adi:`AD9542` can output up to 5 differential (or 10 single-ended) clock signals, plus two single-ended clocks driven by a mix of two high performance digital PLLs. 10 total outputs and 4 reference inputs are accessible on the evaluation board.

The output differential transmission line pairs use 50Ω single ended characteristic impedance and are connected to standard edge launch SMA connectors. The AD9542/PCBZ has a fully configurable power supply to allow the user to evaluate the :adi:`AD9542` while being powered directly by a step down switching regulator or external LDOs. The :adi:`AD9542` evaluation board uses RoHS-compliant FR-4 material. For convenience, detailed information from the AD9542 :adi:`Datasheet <media/en/technical-documentation/data-sheets/AD9542.pdf>` has been included here. Use this user guide in conjunction with the datasheet that has been provided by ADI.
...
|image1|
   ...
   **Figure 1. AD9542 Evaluation Board**

EVALUATION BOARD HARDWARE

The following instructions are for setting up the physical connections to the :adi:`AD9542/PCBZ <EVAL-AD9542>` evaluation board. The user must install the evaluation software prior to connecting the evaluation board for the first time.

Power Connections
~~~~~~~~~~~~~~~~~

The :adi:`AD9542/PCBZ <EVAL-AD9542>` is set up by default to power the :adi:`AD9542` and remaining circuitry using the provided 6V wall power supply.
...
-  A dedicated Analog Devices 3.3V :adi:`ADP7104 Low Noise LDO <ADP7104>` for cases in which the user wishes to evaluate the :adi:`AD9542` with an ultra-quiet power supply.
...
By default, the 3.3V ADP2384A switcher output supply supplies a 1.8V Analog Devices :adi:`ADP7104 Low Noise LDO <ADP7104>` to power the :adi:`AD9542`. The :adi:`ADP2384A Switching Regulator <ADP2384>` can alternately be configured to output 1.8V and directly power the :adi:`AD9542` to evaluate the :adi:`AD9542` when powered with a switching power supply.

-  **AD9542 Powered by ADP2384A Step-down Switching Regulator cascaded with ADP7104 LDO (Default):**
...
-  **AD9542 Powered by 1.8V ADP2384A Step-down Switching Regulator Directly:**
...
Refer to the Evaluation Board Software section for details on running the AD9542
evaluation board software.

Reference Inputs
~~~~~~~~~~~~~~~~

The :adi:`AD9542` evaluation board has four reference inputs sources. By default, REF A/AA (Connectors J300 and J301, respectively) have a transformer so that the user can use a signal generator and use REFA in differential mode.
...
-  Download and install the latest AD9542 ACE Plug-ins: `board.ad9542.1.1.8.zip <https://wiki.analog.com/_media/resources/eval/board.ad9542.1.1.8.zip>`_

(Note that there are a total of four plug-ins that must be installed. Please
unzip it first)

-  Please only use ACE Software version 1.10.2671.1118. Do not update it to
   newer versions as they are not tested with the AD9542 plug-in module
...
The :adi:`AD9542` board view can be seen in **Figure 3**. As the AD9542 evaluation platform is for a single component, the board view has only two main portions, as shown in **Figure 3**:

-  View toolbar
-  AD9542 primaray component link

.. container:: centeralign

   |image3| **Figure 3. AD9542 ACE Plug-in Board View**
...
.. container:: centeralign
   ...
   |image4| **Figure 4. AD9542 ACE Plug-in Chip View**

The chip view is the primary interface used to configure the AD9542. It consists
...
checking, and allows the user to intuitively configure the AD9542 parameters. It
should be noted that in order to synchronize the register content of the
software with that of the AD9542, the user must press the ‘Read All’ button in
...
.. container:: group
   ...
   .. container:: half column
      ...
      The configuration wizard simplifies configuring the AD9542 by providing
...
A detailed explanation of the interaction with the block diagram is explained in the `ACE User Guide <http://swdownloads.analog.com/ACE/ACE_User_Manual_rev3.pdf>`_ sections 3.7.1 through 3.7.3. However, it should be noted that the AD9542 chip view heavily leverages the use of configuration menus for blocks with multiple associated bit fields. Clicking on such a block will pop up a menu as pictured in **Figure 6**.
...
.. container:: centeralign
   ...
   |image8| **Figure 8. AD9542 ACE Plug-in Memory Map View**

The memory map view is explained in detail in the `ACE User Guide <http://swdownloads.analog.com/ACE/ACE_User_Manual_rev3.pdf>`_ section 3.8, but one of the more useful features in the functional group filter shown in the left portion of **Figure 8**. The AD9542 bit fields have been grouped by relevant functionality and the functional group filter allows the user to quickly and easily switch between viewing only bit fields or registers contained within these macro level functional groups.
...
-  Ensure the ACE evaluation software and AD9542 ACE Plug-ins are installed and running.
-  Click File > Open Session…, and select the appropriate .cso file.
-  In the AD9542 Chip View Toolbar, click the Apply Changes button.
...
.. |image1| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542top.jpg
   :width: 700
.. |image2| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542_ace_openingscreen.png
   :width: 700
.. |image3| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542_boardview.png
.. |image4| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542_chip_view.png
.. |image5| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542_configwizard.png
.. |image6| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9542_ace_chip_view_block_configuration_wizard.png
.. |image7| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9545/ad9545_ace_chip_view_frequency_pop_up.png
.. |image8| image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad9542/ad9545_memorymap_bitfields.png

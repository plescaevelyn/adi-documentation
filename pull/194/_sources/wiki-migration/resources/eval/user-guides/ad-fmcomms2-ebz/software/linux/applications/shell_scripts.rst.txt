...
`Shell scripts <https://en.wikipedia.org/wiki/Shell_script>`_ are scripts written for the shell (`dash <https://en.wikipedia.org/wiki/Debian_Almquist_shell>`_, `bash <https://en.wikipedia.org/wiki/Bash_(Unix_shell)>`_, or `ash <https://en.wikipedia.org/wiki/Almquist_shell>`_) of an operating system. To find out which shell, you are using, try something like:

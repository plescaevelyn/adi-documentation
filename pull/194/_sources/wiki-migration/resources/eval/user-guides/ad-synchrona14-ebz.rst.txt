...
|                          | *\*Configurable differential outputs, 50 Ω diff*                 |
+--------------------------+------------------------------------------------------------------+
| Processing System        | Raspberry Pi 4, ARM Cortex-A72, 2 GB SDRAM                       |
+--------------------------+------------------------------------------------------------------+
| Power Supply             | DC 12V, 3A barrel jack                                           |
+--------------------------+------------------------------------------------------------------+
| Internal References      | 100 MHz and 122.88 MHz ultralow phase noise VCXOs (-165 dBc/Hz)  |
|                          | 40 MHz and 38.4 MHz TCXOs (±1 ppm)                               |
|                          | 50 MHz OCXO (±10 ppb)                                            |
|                          | *Internal references are software configurable.*                 |
+--------------------------+------------------------------------------------------------------+
| Clock Processing Devices | HMC7044 (High performance, 3.2 GHz, 14-output Jitter Attenuator) |
...
| ac-coupled |                                                                                                                |
+------------+----------------------------------------------------------------------------------------------------------------+
| For LVPECL | Insert C61, C62, R87, R88, R90, and then DNI the rest of the passive components.                               |
| ac-coupled |                                                                                                                |
+------------+----------------------------------------------------------------------------------------------------------------+
| For LVPECL | Replace C61 and C62 with 0 Ω resistors, insert R87, R88, R90, and then DNI the rest of the passive components. |
| dc-coupled |                                                                                                                |
+------------+----------------------------------------------------------------------------------------------------------------+
| For CMOS   | Replace C61 and C62 with 0 Ω resistors, and then DNI the rest of the passive components.                       |
+------------+----------------------------------------------------------------------------------------------------------------+

Check the :adi:`HMC7044` data sheet (page 21) for more details on output configuration.
...
| **CH3** goes directly into :adi:`HMC7044` and it is dc-coupled.                                             |

+-------------------------------------------------------------------------------------------------------------------------------------------+

| **PPS** is a CMOS input for 1 pps reference. The range for this input is from 1.8 V to 5 V.                                               |

+-------------------------------------------------------------------------------------------------------------------------------------------+

| **SYNC** is a 3.3 V CMOS input that goes directly into :adi:`HMC7044` sync pin.                             |

+-------------------------------------------------------------------------------------------------------------------------------------------+

| **REF_IN** is a 50 Ω input with a maximum input power of 10 dBm.                                                                          |

| With the standard software version, this input is set to function with a 10 MHz reference. The maximum input frequency is 160 MHz (–3dB). |
+-------------------------------------------------------------------------------------------------------------------------------------------+
...
| 
| ==== Internal References ==== There are two **VCXOs**, ultralow phase noise oscillators (-165 dBc/Hz).

The VCXOs are 100 MHz and 122.88 MHz; both of which are software selectable and used to drive the PLL2 of :adi:`HMC7044`.
...
The Raspberry Pi inside AD-SYNCHRONA14-EBZ runs the **RaspAP**, which is an application that gives access to the GUI.
...
-  Connect the add-on board that you received along with the AD-SYNCHRONA14-EBZ, in the back to the GPIO port
...
::
   ...
   eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
           ...
           inet6 fe80::6b61:ce03:6067:283c  prefixlen 64  scopeid 0x20<link>
...
-  Now you need to enable DHCP on SYNCHRONA. You need to do the followings:
...
-  Wait for the device to reboot. When all is done, you should see the initial text like in the code snippet from above
-  Now, typing **ifconfig** should give you a different address from the default one which was inet 192.168.2.1
-  Type the IP in your browser to access the GUI. The default username is **admin** and the password is **analog**.
...
The General Page of the GUI allows users to enable/disable channels, and set the frequencies on each channel. Refer to the **Clock Output Configuration** section above for the proper settings.
...
+----------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Reload data**                  | This button reloads all debug logs and configurations into the GUI.                                                                                                                                                                                                                                                                                                                                                    |
|                                  | Note that this function does not change device configuration.                                                                                                                                                                                                                                                                                                                                                          |
+----------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
The :adi:`AD9545` can provide any clock frequency up to 500 MHz. The differential output OUT1_A of the :adi:`AD9545` is connected directly (ac-coupled) to PLL1 reference input (CLKIN2) of :adi:`HMC7044`. The reference input to PLL2 of :adi:`HMC7044` is connected via an RF switch to two ultralow phase noise VCXOs.
...
.. admonition:: Download
   ...
   `Schematic AD-SYNCHRONA14-EBZ <https://wiki.analog.com/_media/resources/eval/user-guides/ad-synchrona14-ebz/02-064652-01-b.pdf>`_

...
ADXL312 accelerometer. For more information on performance details, see the
ADXL312 datasheet. The EVAL-SDP-CB1Z is the serial system demonstration platform
...
EVAL-ADXL312-SDP is an evaluation board that is separate from the interface
...
The ADXL312 Customer Evaluation System provide an easy start evaluating the
...
As described in the ADXL312 data sheet, a software reset command resets the
ADXL312 and clears all user applied settings. This returns the part to its
...
reset of the ADXL312 device. The software reset command can be issued at any
...
example, when the part first turns on, it powers up in standby mode. The ADXL312
...
device around to observe the response. The ADXL312 contains a 3-axis
...
The ADXL312 has many user configurable settings to customize its operation for
various applications. Configuring the ADXL312 is as simple as selecting the
...
Select the configuration option to dynamically update the ADXL312 device. For
example, changing the sleep data rate from 8 Hz to 1 Hz, as shown in Figure 14,
updates the ADXL312 register map immediately. This way, the ADXL312 Memory Map
clearly shows how the bits correspond to the applied configuration. For more
information on the register map, see the ADXL312 data sheet.
...
There are five interrupts that can be present in the ADXL312 and each
...
of these interrupts, see the ADXL312 datasheet.
...
ADXL312 datasheet). Each interrupt is mapped to INT1 by default. The INT_SOURCE
...
ADXL312 into Auto-Sleep mode in the Overview Tab. The first is to enable the
...
ADXL312. FIFO (or First In, First Out) is a method for utilizing a data buffer
...
Complete the following steps to configure the ADXL312 to operate in FIFO Trigger
...
Now that the ADXL312 is configured, click Start Data Acquisition. Data will be
...
ADXL312 will record 5 data points before the event and 27 data points after the
...
is very useful for low power consumption applications. The ADXL312 can enter a
...
this panel (see Figure 29). This indicator will show whether the ADXL312 is
...
The Inactivity Threshold determines when the ADXL312 enters sleep mode. When the
...
around 0.15g (the value at the instant the ADXL312 wakes up). The thresholds
...
The ADXL312 is a 3-axis accelerometer, and as such can provide inclination
...
the software allows calibration of the ADXL312. Click the Calibrate XL button to
...
The ADXL312 incorporates a self test feature that effectively tests its
...
one second of data from the ADXL312 at the user specified data rate.
...
ADXL312. The device undergoes a hardware reset followed by a series of internal
...
begins. Figure 40 shows an example of the turn-on characteristics of the ADXL312
...
to demonstrate the turn-on behavior of the ADXL312.
...
or write command for the ADXL312 with the correct CRC. Click a choice from the
...
created on the ADXL312 Register Map. The OFSX register will now display the

ADIS1630X EVALUATION ON THE EVAL-ADIS
=====================================

OVERVIEW
--------

The :adi:`ADIS16305` is a low-profile, high-peformance IMU that uses a serial peripheral interface for data communications. This interface enables direct connection with a large variety of embedded processor products. This electrical connection typically only requires 5 I/O lines for sychronous data collection, as show in the following figure:

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-ep-hookup.png
   :width: 400

The :adi:`ADIS16300` offers the same function, package, and interface as the :adi:`ADIS16305`, but it only calibrated at +25C. While the majority of the discussion in this Wiki will refer to the :adi:`ADIS16305` specifically, all of this information also applies to the :adi:`ADIS16300`.

ADIS16305/PCB BREAKOUT BOARD
----------------------------

For those who are on a tight timeline, connecting the :adi:`ADIS16305` to an embedded controller will provide the most flexibility in developing application firmware and will more closely reflect the final system design. The :adi:`ADIS16305/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16305/products/EVAL-ADIS16305/eb.html>` is the breakout board for the :adi:`ADIS16305` and may provide assistance in the process of hooking it up to an existing embedded processor system. For more information on the :adi:`ADIS16305/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16305/products/EVAL-ADIS16305/eb.html>` breakout boards, check out the following links:

:adi:`ADIS16305/PCB Breakout Board Web Page <en/mems-sensors/mems-inertial-measurement-units/adis16305/products/EVAL-ADIS16305/eb.html>`.

:adi:`ADIS16300/PCB Breakout Board Web Page <en/mems-sensors/mems-inertial-measurement-units/adis16300/products/EVAL-ADIS16300/eb.html>`.

EVAL-ADIS: PC EVALUATION
------------------------

For those who would prefer to perform PC-based evaluation of the :adi:`ADIS16305`, before developing their own embedded system, the :adi:`EVAL-ADIS` is the appropriate system to use. The remainder of this Wiki site will focus on PC-based evaluation with the :adi:`EVAL-ADIS` system.
...
NOTE: Newer versions of the .NET framework do not currnently support the IMU
Evaluation software pacakge.

PHYSICAL SETUP
--------------

The :adi:`ADIS16305/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16305/products/EVAL-ADIS16305/eb.html>` includes one interface PCB, one flexible cable/connector and one :adi:`ADIS16305BMLZ <ADIS16305>` unit. Use the flexible cable/connector to install the :adi:`ADIS16305` onto the :adi:`EVAL-ADIS`, using the following five steps. Set interface PCB aside, as it is not used for connecting the :adi:`ADIS16305BMLZ <ADIS16305>` to the :adi:`EVAL-ADIS`.
...
NOTE: Most of the pictures in this section represent the :adi:`ADIS16448`, not the :adi:`ADIS16305`. The packages associated with these two products are close enough to illustrate the key points associated with installing the :adi:`ADIS16305BMLZ <ADIS16305>` on the :adi:`EVAL-ADIS`. The key differences to keep in mind when using these instrucitons are:

1. The :adi:`ADIS16305BMLZ <ADIS16305>` as two mounting holes (instead of two)

2. The :adi:`ADIS16305BMLZ <ADIS16305>` uses the **B** holes on the EVAL-ADIS for attachment.

Step #1
~~~~~~~

Install the flexible cable onto the :adi:`ADIS16305AMLZ <ADIS16305>`. The following pictures provide a visual refernece for correct connection.

|image1| |image2| |image3|

WARNING: Make sure that the connector is in proper alignment before pressing it
in. Misalignment can cause pin damage and exposure to harmful conditions. The
following pictures provide visual examples of INCORRECT PIN ALIGNMENT. The most
common alignment issues will cause the top view to look different than the third
picture, shown directly above this paragraph.
...
Connect the other end of the flexible cable to the :adi:`EVAL-ADIS`, using J4, pins 1-20. J4 has 24 pins, so make sure that the flexible cable connects only to pins 1-20 on J4.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-eval-adis-install-01.jpg
   :width: 600

Step #3
~~~~~~~

Slide the :adi:`ADIS16305BMLZ <ADIS16305>` body towards J4, while tucking the excess flexing under the connector interface, as show in the following pictures.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-eval-adis-install-02.jpg
   :width: 600

Step #4
~~~~~~~

Secure the :adi:`ADIS16305BMLZ <ADIS16305>` body, using (4) M2x0.4x10mm machine screws (included with :adi:`EVAL-ADIS`) and the "D" mounting holes (EVAL-ADIS labels for mounting holes).

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-eval-adis-install-03a.jpg
   :width: 600

Step #5
~~~~~~~

Set JP1 (:adi:`EVAL-ADIS`) to **+5V**. The following picture shows JP1 in the **+3.3V** position
...
.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16334-p33error.png
   :width: 400
...
This section currently has no :adi:`ADIS16305`-specific content, but the :doc:`ADIS16448 Evaluation on the EVAL-ADIS Wiki Site </wiki-migration/resources/eval/user-guides/inertial-mems/imu/adis16448>` has some good examples to start with.

.. |image1| image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-cor-con-01.jpg
   :width: 200
.. |image2| image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-cor-con-02.jpg
   :width: 180
.. |image3| image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-cor-con-03.jpg
   :width: 130
.. |image4| image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-miss-align-con-01.jpg
   :width: 250
.. |image5| image:: https://wiki.analog.com/_media/resources/eval/user-guides/inertial-mems/imu/adis16305-pcb-miss-align-con-02.jpg
   :width: 220

...
The quick start guide is available on the landing page here :doc:`(Quick Start Guide) </wiki-migration/resources/eval/eval-ad7175-8ardz>` or for the step by step install guide see the below.

Installation Guide
------------------

The EVAL-AD7175-8ARDZ evaluation kit includes a link to the software that needs to be installed before using the EVAL-AD7175-8ARDZ evaluation board.
...
To start the ACE evaluation software, from the Windows Start menu, click Analog Devices > ACE. The software window opens (See Figure 5) until the software recognizes the AD7175-8 evaluation board. When the software recognizes the board, double-click on the icon in the **Start** view to open the main window seen in Figure 6. Make sure that you already have the AD717x-8 plugin in the plugin manager.

By clicking on the part in the main ACE evaluation software window (See Figure 6), the chip view will be opened (Figure 7).

The chip view shows the block diagram of the AD7175-8. This tab allows the user to select inputs, set up the ADC, reset the ADC, and view errors present, as well as configure the device for different demonstration modes. Figure 7 shows the chip view in detail, and the following sections discuss the different elements on the Chip view of the software window.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad7175-8/ace_figure_5.png
   :width: 800px

**Figure 5. ACE Evaluation Select Interface Window**

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad7175-8/ace_figure_6.png
   :width: 800px
...
The **Configuration tab** shows a block diagram of the AD7175-8. This tab allows the user to select inputs, set up the ADC, reset the ADC, and view errors present, as well as configure the device.

Functional Block Diagram (1)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

-  The functional block diagram of the AD7175-8 (Label 1 in Figure 7) shows each of the functional blocks within the AD7175-8. Clicking a functional block on this diagram opens the configuration pop-up window for that block.
...
-  There are three external parameters that are set by the EVAL-AD7175-8ARDZ but must be entered into the software: the AVDD1 and AVDD2 (Label 3 in Figure 7), external reference (Label 4 in Figure 7), IOVDD (Label 5 in Figure 7)and AVSS (Label 6 in Figure 7). The external reference on the EVAL-AD7175-8ARDZ is set to 2.5 V by using an ADR4525. If bypassing the ADR4525 on the board, change the external reference voltage value in the software to ensure correct calculation of results in the Waveform and Histogram tabs in the Waveform Analysis window.
...
**Figure 7. AD7175-8 Functional Block Diagram**
...
:doc:`Return to Hardware Guide </wiki-migration/resources/eval/user-guides/eval-ad7175-8ardz/hardware_guide>` :doc:`Return to Homepage </wiki-migration/resources/eval/eval-ad7175-8ardz>`

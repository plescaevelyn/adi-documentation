AD-FMCMOTCON2-EBZ Quick Start Guides
====================================

The Quick Start Guides provide a simple step by step instruction on how to do an initial system setup for the :adi:`AD-FMCMOTCON2-EBZ <en/design-center/evaluation-hardware-and-software/evaluation-boards-kits/Eval-FMCMOTCON2.html#eb-overview>` on various FPGA development boards.

Quick Start Guides are available for

-  :doc:`Hardware setup guide </wiki-migration/resources/eval/user-guides/ad-fmcmotcon2-ebz/quickstart/lv_setup_guide>`
-  :doc:`Linux on ZED </wiki-migration/resources/eval/user-guides/ad-fmcmotcon2-ebz/quickstart/zynq>`

.. image:: https://wiki.analog.com/_media/navigation_ad-fmcmotcon2-ebz#introduction#./
   :alt: Overview#Hardware

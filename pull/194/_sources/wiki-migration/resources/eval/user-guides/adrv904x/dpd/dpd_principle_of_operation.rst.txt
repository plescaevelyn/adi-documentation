...
A baseband model of the power amplifier is created and trained on the input and output digital-baseband samples that pass through the PA as shown in the figure below. The pre-distorter then applies an inverse of the PA model function to input samples before passing them to the transmitter output. The cascade of the pre-distorter response and the PA response becomes a nearly-linear system.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/adrv9029/adrv9029_dpd_principleofoperation.png
   :align: center
   :width: 400px

The intermodulation distortion products between various subcarriers due to PA non-linearities in a wideband transmission protocol such as LTE/NR manifest as power leaked into adjacent channels. Adjacent Channel Leakage Ratio (ACLR) is defined as the ratio of the transmitted power on the assigned channel to the power leaked in the adjacent radio channel. The ACLR performance improvement following the application of DPD to the baseband data is captured in the figure below. These plots illustrate how the out of band non-linearities due to intermodulation products of an LTE 20 MHz signal are reduced by 15-20 dB after the application of DPD.

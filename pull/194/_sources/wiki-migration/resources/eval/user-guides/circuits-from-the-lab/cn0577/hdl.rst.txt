...
.. important::
   ...
   In the master version, the TWOLANES parameter is set to 1, so it works only
   in two-lane output mode.
...
the device can act in different modes, as described below.

.. important::

   The PD_N and TESTPAT jumpers must be disconnected because the signals are
   tied to GPIOs! By default, TWOLANES is set to 1 in HDL code!

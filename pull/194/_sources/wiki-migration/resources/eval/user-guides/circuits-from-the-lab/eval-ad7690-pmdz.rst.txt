...
In the case of the high precision, successive approximation ADC's architecture, it was required to provide low noise external power supplies to obtain datasheet results. The :adi:`AD7690` ADC is driven by precision amplifiers which are also optimized for noise and power. In order to enable those amplifiers to provide zero and full-scale inputs to the ADC, power supplies above and below the ADC input range were needed.
...
-  :git-precision-converters-firmware:`EVAL-AD7690-PMDZ No-OS Driver <projects/ad7689_iio>`

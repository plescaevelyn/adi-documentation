...
After the board connections are made and Scopy is installed, the user can
connect to the GUI following the next steps:
...
-  In the window that opens hit the refresh button highlighted in the following
   image:
...
-  Select the COM port that is connected to the PQM board from the drop down
   menu “PORT NAME” (In the presented case is COM37)
...
-  A message similar with the one highlighted in the following image shows that
...
-  The Rms tab can be activated by clicking on it and afterwards selecting the
   Run button.
...
-  The PQEvents indicator warns the user that a PQ event occurred. The events
   are saved in the log file if logging is active.
...
-  If an event occurs during the session the PQEvents indicator will become
   active. The event can be found in the log file (in the rms tab only the PQ
   events data is logged into the file). The indicator remains on until the user
   clicks on it. This will reset the indicator. Even if the indicator is not
   reset any new event will be registered in the log file.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad-pqmon-sl/scopy13_5.jpg
   :align: center
   :width: 600

-  Logging can be activated only when the measurement is not running. First the
   LOG button needs to be selected. A log directory needs to be specified after
   selecting the button highlighted in the following image.

.. image:: https://wiki.analog.com/_media/resources/eval/user-guides/ad-pqmon-sl/scopy12_2.jpg
   :align: center
   :width: 600

-  A folder where the data will be saved in csv format needs to be specified.
   The file is saved with the following name: "nameofactivewindow_date_time.csv"
...
-  After the folder is selected, the data will be recorded during a session. The
   session starts when the running button is activated and ends when it is
   stopped.
...
-  A snapshot of a rms log file is presented in the following image. As one can
   see, during the presented session several PQevents occurred.
...
-  The PQEvents indicator warns the user that an event occurred. The events are
   saved in the log file if logging is active.
...
-  A snapshot of the harmonics log file containing only harmonics data is
...
-  In the following snapshots PQ events can be observed being intercalated with
...
-  The Settings tab is used to read and set the thresholds and the config
   values. Activate it by selecting it from the right-side menu.
...
-  To modify a parameter select it, change its value to the desired one and
...
Open the DAPLINK. Drag and drop the provided \*.hex file into DAPLINK. The
firmware will be written on the target MCU.

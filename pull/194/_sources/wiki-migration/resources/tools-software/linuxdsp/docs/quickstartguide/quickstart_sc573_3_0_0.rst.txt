...
You can build three different versions of the root filesystem; tiny, minimal and
full. To build the example images invoke bitbake from within the build directory
created previously.
...
<BUILD_DIR>/tmp/deploy/sdk directory. For example, the minimal image on SC573:
...
u-boot-spl-sc573-ezkit.elf ... (gdb) target extended-remote :3333 Remote
...
If you are using the ADSP-SC573-EZKIT with the full image (you ran 'bitbake
adsp-sc5xx-full'):
...
Or if you are using the ADSP-SC573-EZKIT with the minimal image (you ran
'bitbake adsp-sc5xx-minimal'):

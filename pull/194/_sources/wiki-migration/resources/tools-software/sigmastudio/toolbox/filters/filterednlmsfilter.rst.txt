...
FxLMS filter can be used for Active noise cancellation.The Reference input will
be the reference to the noise to be cancelled.The error(residual signal) input
is the acoustic addition of anti-noise and the noise signal near to the error
microphone.The Filtered Reference is the reference signal filtered through the
secondary path.Where the secondary path is the transfer function of the path
after the FxLMS filter output back to FxLMS error input.
...
The coefficient update equation is W := (1- alpha \* Leakage)\* W + alpha \*
error(n) \* XFilterd/ energy

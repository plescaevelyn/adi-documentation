...
modifies the assembly code, so any time you change the*\* Max*\* setting you
must recompile and download the program. The maximum delay value range is
...
To update the delay percentage via an external micro-controller (MCU), the valid
delay percentage value is a fixed point number between 0.0 and 1.0 (0% to 100%).

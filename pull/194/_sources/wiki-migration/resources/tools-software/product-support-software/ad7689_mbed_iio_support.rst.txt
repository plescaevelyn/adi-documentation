...
Mbed) firmware example with Analog Devices AD7689 Evaluation board(s) and SDP-K1
controller board. This example code leverages the ADI developed IIO (Industrial
Input Output) ecosystem to evaluate the AD7689 family devices (AD7689, AD7682,
AD7699 and AD7949 -one at a time) by providing a device debug and data capture
support.
...
(UART) settings as shown below. Click on refresh button and AD7689 device should
...
To capture the data from AD7689 IIO device, simply select the device and
...
-  Pin mapping of controller board (SDP-K1) w.r.t. target evaluation board
   (AD7689).
...
These files define the user configurations for the AD7689, such as SPI
parameters (frequency, mode, etc) and other init parameters used by No-OS
drivers to initialize AD7689 device (active device, input channel type,
reference source, etc) These are the parameters loaded into device when device
is powered-up or power-cycled.

ad7689_data_capture.c
---------------------

This file implements the data capturing logic for the AD7689 device. The
...
specific attributes (related to AD7689 devices) to read/write the device
...
of AD7689 devices. The complete digital interface (to access memory map and

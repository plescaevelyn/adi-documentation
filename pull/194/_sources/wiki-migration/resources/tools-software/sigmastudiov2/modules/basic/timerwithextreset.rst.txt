...
The timer block starts increment the counter value, when the control signal to
Start/Reset pin is non zero and resets the counter value to zero, when the
control signal to Start/Reset pin is zero. The timer continues the counter
increment until it reaches the Specified value in timer input pin and its sets
the flag to one upon completion. The output of the timer block is 0 or 1,
indicating whether the value of timer is reached.
...
again, until the START/RESET pin is set back to one

...
The Index Lookup Table block allows you to access values stored in a lookup
table (LUT). The table contains a matrix of values, with its size determined by
the user. It has two inputs: one for the X index and one for the Y index. Click
‘OpenGrid’ to open the Table Editor window. Values can be entered in linear and
dB scale. Table values can be saved to and restored from file using "Save Table"
and "Load Table" options on the table editor.

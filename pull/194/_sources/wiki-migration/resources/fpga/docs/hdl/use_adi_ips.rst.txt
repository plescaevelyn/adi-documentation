...
Launch a Quartus GUI and select the Tools dropdown from the top menu. Click the Options category. From the pop-up window, select IP Catalog Search Locations and add the path to the library folder inside the ADI repository you just cloned. <code> <hdl_path>/hdl/library/\**/\* </code> Wait for the IP Catalog to refresh and now you should be able to see the IPs under Library/Analog Devices.

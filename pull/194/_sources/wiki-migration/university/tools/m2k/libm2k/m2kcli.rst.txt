...
Build and install libm2k. Instruction can be found :doc:`here </wiki-migration/university/tools/m2k/libm2k/libm2k>`. Make sure to enable tools option.
...
-  Electronics Lab 14 on our wiki page can be found :doc:`here </wiki-migration/university/courses/electronics/electronics-lab-14>`. Configure the connections for 'Unipolar output operation' mode
...
-  Replicate the example found :doc:`here </wiki-migration/university/courses/electronics/m2k-uart-debug>`
...
**The script can be found** :git-libm2k:`here <tools/m2kcli/examples/m2k_tty.sh>`\ **.**
...
-  :doc:`libm2k </wiki-migration/university/tools/m2k/libm2k/libm2k>` with m2kcli

It uses the socat command to create two connected `PTYs <https://linux.die.net/man/7/pty>`_ and using the m2kcli uart-terminal command makes the connection between a PTY and our UART-Terminal. After all of that is done, the user can connect to the other PTY using a Serial Port Terminal Emulator (like GTKTerm, Picocom, Minicom, Tera Term).
...
Sigrok offers two main possibilities of getting the cli. Building from source or downloading the packages. All the steps are well described on their wiki page. For building sigrok-cli have a look at this `page <https://sigrok.org/wiki/Building>`_, while for downloading the binaries and the distribution packages have a look\ `here <https://sigrok.org/wiki/Downloads#Binaries_and_distribution_packages>`_.
...
::
     ...
     m2kcli digital auto -c buffer_size=1000 format=binary | sigrok-cli -i - -I binary:numchannels=16:samplerate=1mhz -P i2c:scl=15:sda=7 -A i2c=address-read:address-write:data-read:data-write

...
|ADLAM-UARTJTAG photo| In a few of the designs we have done, like the :doc:`PlutoSDR </wiki-migration/university/tools/pluto>` and :doc:`M2k </wiki-migration/university/tools/m2k>` there hasn't been enough room on the PCB for serial and JTAG, so we put it on a small breakout board, which communicates to the main platform with a small 10-pin ribbon cable, which is known as the :adi:`ADALM-UARTJTAG`.

This includes the Silicon Labs `CP2103-GM <https://www.silabs.com/Support%20Documents/TechnicalDocs/CP2103.pdf>`_ USB to UART bridge, which you can plug a USB cable into.

This also includes connection to Xilinx JTAG cable as well. You will need a `Xilinx JTAG Cable <https://www.xilinx.com/support/documentation/data_sheets/ds593.pdf>`_ or `equivalent <https://store.digilentinc.com/jtag-hs3-programming-cable>`_ to use it.

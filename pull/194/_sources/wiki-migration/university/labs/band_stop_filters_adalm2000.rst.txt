...
The objective of this Lab activity is to construct a Band Stop Filter by
combining a low pass filter and a high pass filter and obtain the frequency
response of the filter. A Series LC circuit will be used.
...
1.0 KΩ resistor 1 0.1 µF Capacitor(marked 104) 1 20 mH inductor or 2 (10 mH in
series)

Hardware setup
--------------

Build on your solderless breadboard the Band reject filter circuit presented in
Figure 3.
...
The band stop filter's frequency response can be plotted using the Network
...
sweep. For this filter the center frequency is 3.3 KHz. In the network analyzer
set the start frequency at 1 KHz and the stop frequency at 10 KHz. Set the
...
frequency value in the rejection interval of the filter and analyze it's
response. Observe on the oscilloscope channel 1 the input signal and the output
signal on channel 2. In Figure 6 you can see the filter input and output for a
3kHz sine waveform.
...
.. container:: centeralign
   ...
   Figure 6:Input and output signals of Band reject filter circuit for 3kHz
   input frequency

Questions
---------

Compute the cut-off frequencies for each Band reject filter constructed using

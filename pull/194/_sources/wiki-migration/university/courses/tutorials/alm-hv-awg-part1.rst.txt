...
The objective of this document is to present various techniques that can be used
to increase the output voltage available from a Personal Test Instrument signal
generator or AWG.
...
Even the most well equipped labs with large budgets may not have the exact right
instrument for a particular test or measurement. What do you do in this
...
certain components and IC's require higher voltages. Can this be done only using
USB powered personal instruments like ADALM1000 (M1k) / ADALM2000 (M2k)?
...
is by using a step-up transformer. While the voltage seen on the output
...
A selection of AC power transformers is listed in table 2. The DC resistance of
the two windings is listed along with the voltage step down (up) ratio. Lastly
the open circuit peek-to-peek voltage in a step up configuration is listed for
an input p-p voltage of 5 V.
...
resistors in series (440 K) and two 22 K resistors in parallel (11 K) attenuates
the waveform so it can be monitored by the 0-5 V input range of the M1k.
...
tri wave) used for triggering and reference. The brown and darker cyan reference
traces show the high voltage and AWG drive current without the neon lamp in
circuit. The orange and cyan traces are with the lamp in circuit. You can see
clearly in the voltage and current traces where the lamp turns on (around 113 V)
and the current jumps above the reference base line and where it extinguishes
(around 70 V). The big negative current spike is when the diode turns on and
...
Performing the same experiment using the M2k hardware is shown next in figure 8.
Both AWG channels are used in parallel to drive the primary side of the
transformer. The two 50Ω resistors inside the blue dashed box represent the
...
effectively lowers the source resistance to 25Ω and boosts the effective voltage
and current that is applied to the transformer. Otherwise the experimental setup
is the same. (Inputs 1- and 2- assumed to be grounded). The maximum voltage
swing achievable is about 15V lower due to losses in the 25Ω source resistance.
...
is the raw voltage seen at the 41:1 resistor divider and the green math trace is
the scaled high voltage waveform. The M2k lacks a current measurement channel so
it is more difficult to see where the lamp turns on and where it extinguishes.
There is a noticeable jump in the voltage trace when the lamp turns on.
...
.. |image1| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig2.png
   :width: 400
.. |image2| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig4.png
   :width: 400
.. |image3| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig5.png
   :width: 400
.. |image4| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig6.png
   :width: 600
.. |image5| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig7.png
   :width: 700
.. |image6| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig8.png
   :width: 600
.. |image7| image:: https://wiki.analog.com/_media/university/courses/tutorials/hv-awg-part1-fig9.png
   :width: 700

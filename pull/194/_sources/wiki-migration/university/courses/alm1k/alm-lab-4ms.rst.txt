...
A MOS FET (NMOS) device can be used in many circuit configurations such as an
amplifier, oscillator, filter, rectifier or just used as an on-off switch. If
the FET is biased into the saturation region, it will operate as an amplifier or
other linear circuit, if biased alternately in the linear (triode) region and
cut-off region, then it is being used as a switch, allowing current to flow or
not to flow in other parts of the circuit. This lab activity describes the NMOS
device when operated as a switch.
...
input. Conditions 2 assure that the transistor will be driven into the triode
region.
...
One common application for a NMOS (or any other) switch is to drive an LED. An
...
circuit goes high (+3 V), the transistor is driven into triode and the LED
...
Save the voltage trace across the transistor Drain-Source ( channel B dashed
green line ) and at the LED ( channel B solid green line ) and include them in
your lab write-up.
...
Two NMOS transistors can be connected with their drains and sources in parallel,
figure 3, which provides a way to switch on the load from two different signals.
Either input can turn on the load but both need to be off for the load to be
off. This is referred to as an “OR” function.
...
Two NMOS transistors can be connected in series with the drain of the lower
transistor connected to the source of the upper transistor, figure 4, which
...
referred to as an “AND” function.

...
The objective of this lab activity is to investigate current sensing techniques
that use an op-amp configured as a difference amplifier.
...
The load will consist of different impedances such as a resistor, capacitor or
inductor.
...
Set CH-A to SVMI Mode and Shape sine. Set the Min value to 0.5 V and the Max
value to 4.5 V. Set the frequency of AWG channel A to 5000 Hz. Set CH-B to Hi-Z
mode.
...
Now compare the output (CB-V) trace for this step with your saved output traces
...
Now compare the output (CB-H) trace for this step with your saved output traces
from steps 1, 2 and 3. Include this and the other plots in your lab report.

Extra:
~~~~~~

As an extra check on your results you can turn on the CA-I curve which measures
the current flowing in the output of the channel A AWG generator. The
measurement made on the ALM1000 board is done in exactly the same way with a
shunt resistor but uses the AD8210 current sense amplifier.

...
.. code-block:: bash
   ...
   analog@analog:~$ curl -fsSL https://get.docker.com -o get-docker.sh
   analog@analog:~$ sudo sh get-docker.sh
   analog@analog:~$ sudo usermod -aG docker $USER

Log out and back in (or run ``newgrp docker``) for the group membership to take
effect.

Build the ADI ROS2 Base Image
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Clone the `adi_ros2 <https://github.com/analogdevicesinc/adi_ros2>`__ repository
and build the base Docker image. On ADI Kuiper Linux (which ships with ROS2
Lyrical), pass the ``ROS_DISTRO`` build argument:

.. code-block:: bash

   analog@analog:~$ git clone https://github.com/analogdevicesinc/adi_ros2.git
   analog@analog:~$ cd adi_ros2
   analog@analog:~/adi_ros2$ docker compose -f compose.build.yml build --build-arg ROS_DISTRO=lyrical base

.. note::

   The ``--build-arg ROS_DISTRO=lyrical`` flag is required on current Kuiper
   images. Omitting it defaults to Humble, which will produce images
   incompatible with the Lyrical-based demo containers.
...
Navigate to the example directory and build the image, again passing the
``ROS_DISTRO`` argument:

.. code-block:: bash

   analog@analog:~$ cd iio_ros2/examples/adalm_lsmspg/docker
   analog@analog:~/iio_ros2/examples/adalm_lsmspg/docker$ docker compose build --build-arg ROS_DISTRO=lyrical
...
the system. **Open a second terminal** (the first terminal must remain running
``docker compose up``) and start a shell inside the Docker environment:

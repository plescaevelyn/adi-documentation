.. imported from: https://wiki.analog.com/resources/eval/user-guides/ad-fmcomms2-ebz/software/baremetal

.. _ad-fmcomms2-ebz software baremetal:
...
.. list-table::
   ...
   * - Switches between internal and external LO. Receives as parameters a
       structure that contains the AD9361 current state and the desired option
...
.. list-table::
   ...
   * - Gets the RX FIR filter configuration. Receives as parameters a structure
       that contains the AD9361 current state, the desired channel (RX1, RX2)
...
.. list-table::
   ...
   * - Sets the RX RF input port. Receives as parameters a structure that
       contains the AD9361 current state and the desired port (A_BALANCED,
...
.. list-table::
   ...
   * - Gets the selected RX RF input port. Receives as parameters a structure
       that contains the AD9361 current state and the location to store the
...
.. list-table::
   ...
   * - Recalls specified RX fastlock profile. When in fastlock pin select mode
       (init_param->rx_fastlock_pincontrol_enable), the function needs to be
       called before then the pin-control can be used. Receives as parameters a
       structure that contains the AD9361 current state, the profile number (0 -
...
.. list-table::
   ...
   * - Saves RX fastlock profile. In order to use more than 8 Profiles, an
       ...
       state, the profile number (0 - 7) and the location for storing the
...
.. list-table::
   ...
   * - Gets the RX Local Oscillator power status. Receives as parameters a
       structure that contains the AD9361 current state and the location to
...
.. list-table::
   ...
   * - Gets the TX FIR filter configuration. Receives as parameters a structure
       that contains the AD9361 current state and the location to store the FIR
...
.. list-table::
   ...
   * - Gets the selected TX RF output port. Receives as parameters a structure
       that contains the AD9361 current state and the location for storing the
...
.. list-table::
   ...
   * - Gets the status of the auto calibration flag. Receives as parameters a
       structure that contains the AD9361 current state and the location for
...
.. list-table::
   ...
   * - Stores TX fastlock profile. Receives as parameters a structure that
...
.. list-table::
   ...
   * - Recalls specified TX fastlock profile. Receives as parameters a structure
...
.. list-table::
   ...
   * - Do multi chip synchronization. Receives as parameters a structure that
       contains the AD9361 master current state and a structure that contains
...
.. list-table::
   ...
   * - Set the OSR rate governor. Receives as parameters a structure that
       contains the AD9361 current state and the desired OSR rate governor
...
.. list-table::
   ...
   * - Gets the OSR rate governor. Receives as parameters a structure that
       contains the AD9361 master current state and the location for storing the
...
.. list-table::
   ...
   * - Gets the temperature. Receives as parameters a structure that contains
       the AD9361 current state and the location for storing the current
       temperature (degrees C \* 1000). Returns 0 in case of success, negative
       error code otherwise.

.. _eval-ad35xxr evb quickstart zed:
...
- :adi:`EVAL-AD3552RFMCxZ <EVAL-AD3552R>` evaluation board
...
#. Copy the boot files (``BOOT.BIN``, ``uImage``, ``devicetree.dtb``)
   ...
   .. collapsible:: Build from source
      ...
      **HDL reference design**
      ...
      The bitstream is written into ``hdl/projects/ad3552r_fmcz/zed/ad3552r_fmcz_zed.sdk/``.

      **Kuiper Linux kernel and device tree**

      Refer to :external+kuiper:ref:`use-kuiper-image` for building
      ``uImage`` and ``devicetree.dtb`` for Zynq with the AD3552R device
      tree overlay.
...
#. Configure the boot mode and VADJ voltage. The ZedBoard jumpers should
   be set for SD card boot and VADJ = 1.8 V.

   .. important::

      Set VADJ to **1.8 V** before powering on. The AD3552R uses 1.8 V
      signaling on the FMC connector; a higher VADJ voltage can damage the
      evaluation board.

   .. image:: ../images/zed_vadj_sd_boot.jpeg
      :alt: ZedBoard jumper configuration for SD card boot and VADJ = 1.8 V
      :align: center
      :width: 500

#. With the ZedBoard **powered off**, insert the
   :adi:`EVAL-AD3552R` evaluation board into the FMC LPC connector.
...
.. figure:: ../images/ad35xxr_setup_linux.jpeg
   :alt: EVAL-AD3552R hardware setup on ZedBoard
   :align: center
   :width: 700

   EVAL-AD3552R hardware setup on ZedBoard.

.. image:: ../images/setup_diagram_eval_ad3552r.jpeg
   :alt: EVAL-AD3552R hardware setup diagram showing ZedBoard connections
   :align: center
   :width: 900
...
#. Open IIO Oscilloscope. In the connection dialog, enter the URI of the
   ZedBoard:
   ...
   .. note::
      ...
      The host PC and ZedBoard must be on the same network for IP-based
      access. You can verify connectivity with
      ``iio_info -u ip:<ip-address-of-zedboard>`` from the host PC.

#. Click **Refresh** to populate the device list, then click **Connect**.

   .. figure:: ../images/iio_connect.png
      :alt: IIO Oscilloscope connection dialog showing the ZedBoard URI
      :align: center
      :width: 1000

      IIO Oscilloscope - connecting to the ZedBoard

#. In the DAC plugin, select the ``axi-ad3552r`` device. You can set
   raw output codes for each channel or configure DDS tone generation.

   .. image:: ../images/output_raw_1_.jpeg
      :alt: IIO Oscilloscope showing raw voltage output values for AD3552R
      :align: center
      :width: 600

#. Use the **Debug** tab to access individual IIO attributes directly,
   such as ``output_range``, ``sampling_frequency``, and
   ``out_voltage0_raw``.

   .. figure:: ../images/iio_debug.png
      :alt: IIO Oscilloscope debug tab showing AD3552R attributes
      :align: center
      :width: 600

      IIO Oscilloscope - debug attribute access

.. note::

   If the sine wave on the oscilloscope looks split, this is a data format
   mismatch. The AXI IP core defaults to offset binary, but if your
   software sends two's complement samples (or vice versa), the output
   wraps around like this.
...
#. Install :external+scopy:doc:`Scopy <index>` on the host PC.

#. Connect to the ZedBoard using the network URI
   ``ip:<ip-address-of-zedboard>``.

#. Open the **IIO plugin** in Scopy to read and write IIO device
   attributes, or use the **Signal Generator** plugin to stream samples
   to the DAC.

   .. figure:: ../images/scopy_capture_linux.png
      :alt: Scopy connected to the ZedBoard showing AD3552R channels
      :align: center
      :width: 700

      Scopy - IIO device view
...
.. image:: ../images/python_plot_ad3552r.jpeg
   :alt: PyADI-IIO matplotlib output showing AD3552R sine wave
   :align: center
   :width: 500
...
#. The hardware setup is the same as for the Linux instructions above.
   You need an extra micro USB cable to connect the ZedBoard's JTAG port to
   the host PC in order to flash the No-OS firmware.

   .. figure:: ../images/ad35xxr_setup_linux_no_os.jpeg
      :alt: EVAL-AD3552R hardware setup on ZedBoard
      :align: center
      :width: 700
...
#. Program the ZedBoard with the No-OS firmware (built from
   :git-no-OS:`projects/ad3552r_fmcz`).
...
#. Close the serial terminal, then connect IIO Oscilloscope using the
   serial backend.

   .. figure:: ../images/iiio_connect_no_os.png
      :alt: IIO Oscilloscope connecting to the No-OS IIOD server
      :align: center
      :width: 600

      IIO Oscilloscope - No-OS IIOD serial connection

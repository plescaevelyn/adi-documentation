.. _eval-cn0506-fmcz quickstart a10soc:
...
- :adi:`EVAL-CN0506-FMCZ` FMC evaluation board
...
The following files are needed for the system to boot and should be moved to 
the root of the SD Card partition:
...
- Files from **socfpga_arria10_socdk_cn0506_mii**
  ...
  - Write ``u-boot-splx4.sfp`` from **socfpga_arria10_socdk_cn0506_mii** folder 
    on **third** SD Card partition:

- Files from **socfpga_arria10_common**

  - Linux Kernel image: ``zImage``
  - Move ``extlinux.conf`` in the **extlinux** folder in the root of the SD 
    Card partition (if it doesn't exist you should create it)
...
- :external+hdl:ref:`build_intel_boot_image` (section: Using Kuiper Linux Pre-Built Images)

Instructions on how to manually build the boot files from source can be found
here:

- :external+hdl:ref:`build_intel_boot_image` (section: Examples of building the boot image)
...
To be compatible with the :adi:`EVAL-CN0506-FMCZ`, these pins need to be 
...
to be compatible with the :adi:`EVAL-CN0506-FMCZ`.

.. image:: ../images/a10soc_rework.jpeg
   :align: center
   :width: 500
...
.. image:: ../images/a10soc_setup_cn0506.jpeg
   :align: center
   :width: 800
...
#. Both the HPS (J26) and FPGA (J27) memory module must be installed on the
   A10SoC
#. Connect the :adi:`EVAL-CN0506-FMCZ` FMC board to the A10SOC **HPC1** FMCA 
   (J29) socket

   .. image:: ../images/a10soc_cn0506.jpeg
      :align: center
      :width: 500

#. Insert microSD card into the card socket on the FPGA

   .. image:: ../images/a10soc_sdcard.jpeg
      :align: center
      :width: 500

#. Configure the FPGA for SD card boot mode. You can find the default position
   :intel:`here <content/www/us/en/docs/programmable/683735/current/boot-select.html>`
#. Plug-in an Ethernet cable from your router/switch to whichever Ethernet port
   you prefer on the :adi:`EVAL-CN0506-FMCZ`

   .. image:: ../images/a10soc_eth.jpeg
      :align: center
      :width: 500

#. Connect USB UART1 (J10) to your host PC using the Mini USB cable

   .. image:: ../images/a10soc_usb.jpeg
      :align: center
      :width: 500
...
#. Connect the power supply for the FPGA
   ...
   .. image:: ../images/a10soc_power.jpeg
      :align: center
      :width: 500
...
#. Connect the UART USB Cable connection from A10SoC to your Host PC. Your Host
...
#. Bridge network connections so we can install necessary tools such as iPerf3
   for A10SoC through the Internet. Go to Control Panel > Network and Internet >
...
#. In your UART terminal (TeraTerm), get the IP address of the connected 
   Ethernet port of CN0506. Use the command "ip addr". Take note of
...
#. In your UART terminal (TeraTerm), you will need to run the iperf3 client for 
   each Ethernet port so you can verify its performance by using "iperf3 -c <IP 
   address of server> -B <IP address of client>. Enter the following commands:

...
The EVAL-AD4130-8-PMDZ features three interrupt pins that can be used as
external indicators for user-defined events. These PMOD interrupt pins are
internally connected to the single interrupt output of the AD4130, and can be
configured via software to signal either a Data Ready or FIFO Interrupt
condition.
...
**AD4130 no-OS Driver**
   ...
   * The :git-no-OS:`AD4130 no-OS driver <drivers/afe/ad413x>` is used
     in bare-metal applications, typically running on low-power, embedded
     microcontrollers.

   * The :git-no-OS:`AD4130 no-OS example project <projects/ad413x>`
     uses the AD4130 no-OS driver and has several configuration options:

.. admonition:: Download

   A zip file containing prebuilt programming files for the no-OS example
   project, targeting platforms below are available at:
   :git-no-OS:`ad413x.zip <releases/download/last_commit/ad413x.zip+>`.

   More details are provided in the platform-specific sections below.

**AD4130 Linux Driver**

* The :git-linux:`AD4130 Linux driver <drivers/iio/adc/ad4130.c>` is
  used in applications running the Linux operating system, typically on
  larger processors and SoC devices.
* The AD4130 Linux driver uses the Industrial Input/Output (IIO) framework,
  greatly simplifying the development of application code via the
  ...
  directly on the platform board, communicating with the device over the
  local backend, or from a remote host over the network or USB backends.
...
#. Attach the EVAL-AD4130-8-PMDZ to the Raspberry Pi Interposer Board via Port
P1.
...
To look at the context attributes on the Raspberry Pi, enter the following
...
Click and **+** icon to add a device and enter the URI of your device and then
click verify.
...
.. tip::
   ...
   latest videos, and more when you

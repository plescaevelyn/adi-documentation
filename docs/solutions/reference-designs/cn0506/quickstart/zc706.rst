Quickstart: Setup your CN0506 in ZC706
======================================

This guide provides instructions on how to set up the :adi:`CN0506` on the `ZC706 <https://www.xilinx.com/ZC706>`_ platform.

.. image:: ../images/v_dsc0048.jpg
   :align: center
   :width: 600
...
The CN0506 is a 10 Mbps/100 Mbps/1000 Mbps dual channel, low latency, low power
industrial Ethernet PHY evaluation board designed to interface the MAC and PHY
through MII, RMII, or RGMII interfaces which can be installed on Field
Programmable Gate Array (FPGA) platforms such as ZC706 evaluation board using
VITA-57 FPGA mezzanine cards (FMC) to the low pin count (LPC) FMC connector to
provide a hardware environment for developing and evaluating designs targeting
the Zynq®-7000 XC7Z045-2FFG900C SoC in industrial Ethernet applications.
...
-  Xilinx `ZC706 <https://www.xilinx.com/ZC706>`_ Evaluation Board Rev 1.0 or a later version
...
-  iPerf3 `Download <https://iperf.fr/iperf-download.php>`_

Hardware Setup
--------------

1. Connect the EVAL-CN0506-FMCZ to the low pin count (LPC) FMC connector on the
   ZC706 evaluation board.

.. image:: ../images/dsc0021.jpg
   :align: center
   :width: 600

2. Connect the ZC706 USB-to-UART Bridge with Mini USB Connector (U52, J21) to
   the host PC using a Mini USB cable.

.. image:: ../images/dsc0024.jpg
   :align: center
   :width: 600

3. Connect the Ethernet cable in the CN0506 M1 or M2 Ethernet port depending on
   your preference.

.. image:: ../images/v_dsc0053.jpg
   :align: center
   :width: 600

4. Plug the power cord into the 12V power jack 2x6 connector J22.

.. important::

   Do not switch on yet the Power On/Off Switch (SW1) and make sure that the switch and jumper settings are in default mode. Refer to the `ZC706 User Guide <https://www.xilinx.com/support/documentation/boards_and_kits/zc706/ug954-zc706-eval-board-xc7z045-ap-soc.pdf>`_ for detailed default jumper settings information.

.. image:: ../images/dsc0023.jpg
   :align: center
   :width: 600
...
1. Download the latest Linux Image and flash the file to your 16 GB SD Card. Refer to `Kuiper Images <https://wiki.analog.com/resources/tools-software/linux-software/kuiper-linux>`_ for more detailed instruction.

2. In your flashed Linux Image BOOT partition, copy the kernel to the root
...
3. Copy the boot files to the root folder. The boot files can be found in the
...
.. important::
   ...
   the following files to communicate with the FPGA platform properly using the
   Analog Devices Linux Kuiper OS.

.. image:: ../images/screenshot_2021-11-26_195656.png
   :alt: Kernel and boot files in the root folder
   :align: center
   :width: 600

4. Safely eject the SD Card from your host PC and insert it into your ZC706 SD
   Card Interface Connector (J30).

.. image:: ../images/dsc0016.jpg
   :align: center
   :width: 600
...
1. Make sure you have installed your favorite terminal emulator (in this case,
   TeraTerm) on your Host PC.

2. Connect the UART USB Cable connection from ZC706 to your Host PC. Your Host
...
3. Connect an Ethernet Cable connection from CN0506 M1 to your Ethernet-capable
   Host PC. Make sure the Host PC is also connected to the Internet using Wi-Fi
   or Ethernet.

.. image:: ../images/v_dsc0063.jpg
   :align: center
   :width: 600

4. Bridge network connections so we can install necessary tools such as iPerf3
...
5. Launch TeraTerm and make a new serial connection with the designated COM
...
6. Change the baud rate from 9600 to 115200. Go to Setup > Serial Port... A
...
7. Install iPerf3 on your Linux machine. Enter in the terminal line, then write
   and enter
...
9. Open your Host PC's terminal (e.g. Windows PowerShell) and navigate through
...
10. Get the IP address of the connected Ethernet port of your Host PC using the
    ...
    address 169.254.6.62 of "Ethernet adapter Ethernet" which corresponds to
    your Host PC's Ethernet port.
...
11. In your Linux machine UART terminal (TeraTerm), Get the IP address of the
...
12. In your Host PC's terminal. run the iperf3 as the server. In this case of
...
13. In your Linux terminal (which we use is TeraTerm), you will need to run the
...
15. We're done with the basic iPerf3 performance test! If you want to go further
    with the test, this is your time to shine and this is high time for you to
    get started with your application development.
...
:doc:`CN0506 HDL Reference Designs </solutions/reference-designs/cn0506/hdl>`
...
-  :ez:`Analog Devices Circuit from the Lab Help Forum <community/circuits_from_the_lab>`

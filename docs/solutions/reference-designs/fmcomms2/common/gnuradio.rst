...
signal processing blocks to implement software radios or other generic
processing. This articles focuses on using IIO based devices like PlutoSDR,
:adi:`AD-FMCOMMS2-EBZ`, :adi:`AD-FMCOMMS3-EBZ`, :adi:`AD-FMCOMMS4-EBZ`,
:dokuwiki:`ARRADIO <resources/eval/user-guides/arradio>` and
:adi:`AD-FMCOMMS5-EBZ` within GNU Radio itself. This support is currently
provided in an out-of-tree (OOT) module called **gr-iio**.
...
- :ref:`libiio <libiio>`
- :git-libad9361-iio:`libad9361 </>`
- GNU Radio and its development packages
- swig (Optional for python support)

Since GNU Radio can come from many different sources and with many different
packages includes we will try to cover all the necessary dependencies but this
can differ depending on OS packaging. If you have built and installed gnuradio
from source yourself you should be good to go. Otherwise, consult the
`GNU Radio Wiki <https://wiki.gnuradio.org/index.php/InstallingGR>`_ for further
documentation on the development installation.

Most dependencies do have pre-built binaries available on github
(:git-libiio:`libiio releases <releases+>`, :git-libad9361-iio:`libad9361
releases <releases+>`) or can be build from source as below.
...
::
   ...
   git clone https://github.com/analogdevicesinc/libiio -b v0.25
   cd libiio
   mkdir build
   cd build
...
Build and install libad9361-iio from source:
...
.. important::
   ...
   We would highly recommend upgrading to gr-3.8+. :git-gr-iio:`gr-iio
   <master:/>` was accepted into mainline gnuradio and will be directly provided
   with it from 3.10+
...
to get the latest and most feature complete work, its recommend to build it from
the latest github sources.
...
Since GNU Radio 3.8 is not fully mainstream across package managers 3.8+ support
requires a non-master branch, specifically upgrade-3.8. To get GNU Radio 3.8+
consult the `GNU Radio Wiki - Ubuntu PPA Installation
<https://wiki.gnuradio.org/index.php/InstallingGR#Ubuntu_PPA_Installation>`_.
...
::
   ...
   git clone -b upgrade-3.8 https://github.com/analogdevicesinc/gr-iio
   cd gr-iio
   cmake .
   make
   sudo make install
...
``/usr/local/lib${type}/python${PYVER}/site-packages/gnuradio`` or
``/usr/local/lib${type}/python${PYVER}/dist-packages/gnuradio`` in your
PYTHONPATH during installation. If this is not the case you will need to modify
the cmake command for the gr-iio configuration above with:
...
GNU Radio and the gr-iio blocks are installed by default. If you want to install
GNU Radio on your host consult the official
`GNU Radio Wiki <https://wiki.gnuradio.org/index.php/InstallingGR>`_.

Windows Support
-------------------------------------------------------------------------------

The current recommended way to use GNU Radio under Windows is with the Windows
Subsystem for Linux (WSL). To install WSL follow the instructions from Microsoft
here:
`How to install Linux on Windows with WSL <https://docs.microsoft.com/en-us/windows/wsl/install-win10>`_ .
Please install Ubuntu when selecting an operating system. Note that GR 3.7 has
only been tested on WSL.

Once Ubuntu is up and running go to the `Linux Installation`_ section above and
proceed with "the preferred way" install, which will be much faster than
pybombs.

Once all the blocks are installed an X11 server will need to be install on
Windows itself so we can use GNU Radio's GUI tools. Download and install
`Xming <https://sourceforge.net/projects/xming/>`_ in Windows.
...
``gnuradio-companion``. This should launch the familiar GUI for GNU Radio.

Native
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A complete GNU Radio installer with gr-iio support is provided for
:dokuwiki:`Windows 7/10
<resources/tools-software/linux-software/gnuradio_windows>`. Please read the
entire linked page before installing.
...
- **IIO context URI**: Set to "local:" if using GNU Radio locally on the target
  e.g. Zedboard. If using GNU Radio remote on a PC, set the target IP address
  using ip:XXX.XXX.XXX.XXX (some boards like the PlutoSDR run a Zeroconf/Avahi
  deamon so the the URI may look like this: ip:pluto.local) or via USB using the
  URI usb:XX.XX.XX
- **Buffer size**: Size of the internal buffer in samples. The IIO blocks will
  only input/output one buffer of samples at a time.
...
- **RF Bandwidth(MHz):** Configures RX analog filters: RX TIA LPF and RX BB LPF.
  :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **Sample Rate(MSPS):** Frequency at which the hardware will input/output
  samples. :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **LO Frequency(MHz):** Selects the RX local oscillator frequency. Range 70MHz
  to 6GHz with 1Hz tuning granularity. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
- **RF Port Select:** Selects the RF port to be used. Can be either any of the
  ...
  input. :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **Tracking** :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
...
- **Manual Gain(dB):** Controls the RX gain only in Manual Gain Control Mode
  (MGC). :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **Gain Mode (RX1, RX2):** Selects one of the available modes: manual,
  slow_attack, hybrid and fast_attack. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
...
- **RF Bandwidth(MHz):** Configures TX analog filters: TX BB LPF and TX
  Secondary LPF. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
- **Sample Rate(MSPS):** Frequency at which the hardware will input/output
  samples. :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **LO Frequency(MHz):** Selects the TX local oscillator frequency. Range 70MHz
  to 6GHz with 1Hz tuning granularity. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
- **RF Port Select:** Selects the RF port to be used. :external+linux:doc:`Read
  More <drivers/iio-transceiver/ad9361>`
...
- **Cyclic**: Set to "true" if the "cyclic" mode is desired. In this case, the
  ...
  its processing as complete: the blocks connected to the FMCOMMS-2 IIO block
...
- **Gain Mode:** Selects one of the available modes: manual, slow_attack, hybrid
  and fast_attack. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
- **Filter:** Allows a FIR filter configuration to be loaded from a file.
  :external+linux:doc:`Read More <drivers/iio-transceiver/ad9361>`
- **Filter auto** When enabled loads a default filter and thereby enables lower
...
- **Attenuation (dB):** Individually controlls attenuation for TX1 and TX2. The
  range is from 0 to -89.75 dB in 0.25dB steps. :external+linux:doc:`Read More
  <drivers/iio-transceiver/ad9361>`
- **Cyclic**: Set to "true" if the "cyclic" mode is desired. In this case, the
  first buffer of samples will be repeated on the enabled channels of the
  FMCOMMS-2 until the program is stopped. The FMCOMMS-2 IIO block will report

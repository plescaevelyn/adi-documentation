...
The program we will use, :ref:`IIO Oscilloscope <iio-oscilloscope>`, use the
sysfs interface to configure the ad9361, and use the special character device
/dev/iio:device1 to read the raw values.

Equipment used
-------------------------------------------------------------------------------

This test has been realized with a
`Zed board <http://zedboard.org/content/overview>`__, coupled with a
:adi:`AD-FMCOMMS3-EBZ <ad-fmcomms3-ebz>`
module.

.. image:: ../images/fm_radio_board2.jpg
   :width: 600

Monitoring the FM band
-------------------------------------------------------------------------------

We will monitor the most used FM band, from 87.5 MHz to 107.9 MHz.
The :ref:`IIO Oscilloscope <iio-oscilloscope>` can be configured like this:

.. image:: ../images/fm_radio_capture4.png
   :width: 600

- **RX LO Frequency**: the base frequency at which the hardware will be tuned.
  Set to 87.4 MHz (and not 87.5, since 87.5 is the middle of a 200 KHz channel);
- **RF Baudwidth**: the size of the band you want to capture. Here, we want at
  least a band of 107.9 - 87.5 = 20.4 MHz
- **Sampling rate**: the number of samples you want to capture (in millions per
  second). Here we set this to 22, so that we get a resolution of
  approximatively one hertz.

Lanch a capture, and you will be able to see peaks in the frequency domain:

.. image:: ../images/fm_radio_capture3.png
   :width: 600

In this screenshot, it is clearly possible to identify several channels: at
95.5 MHz, 96.3 MHz, 97.5 MHz, 98.4 MHz among other frequencies.

...
The **CN0395_example** is a volatile organic compounds (VOC) detector demo project for the Arduino Uno base board with additional `EVAL-CN0395-ARDZ Shield <https://wiki.analog.com/resources/eval/user-guides/eval-adicup360/hardware/cn0395>`_, created using the Arduino Genuino IDE.
...
The **CN0395_example** project uses the `EVAL-CN0395-ARDZ Shield <https://wiki.analog.com/resources/eval/user-guides/eval-adicup360/hardware/cn0395>`_ which is a portable VOC detector which comes with a Figaro TGS8100 MOX sensor.

.. image:: ../images/eval-cn0395-ardz.jpg
   :align: center

The TGS8100 sensor requires two voltage inputs: heater voltage (VH) and circuit voltage (VC). The heater voltage (VH) is applied to the integrated heater in order to maintain the sensing element at a specific temperature which is optimal for sensing. The `EVAL-CN0395-ARDZ Shield <https://wiki.analog.com/resources/eval/user-guides/eval-adicup360/hardware/cn0395>`_ circuit provides the heater voltage (VH), by using :adi:`ADN8810` IDAC as a programmable current source. The default full scale current in the IDAC is 9.94mA and the default value of the RSN resistors is 41.2Ω.

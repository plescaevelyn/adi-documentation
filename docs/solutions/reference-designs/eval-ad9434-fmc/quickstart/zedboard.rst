...
- :external+hdl:ref:`ad9434_fmc` build documentation. More HDL build
...
- :adi:`EVAL-AD9434` FMC evaluation board
- SD card with at least 16GB of memory
- Mini-USB cable (UART)
- LAN cable (Ethernet)
- SMA cables

  - 1x for the input, in case of not using an external clock source (see :ref:`ad9434 user-guide`)
  - 2x in case of using and external clock source, one for CLKIN input port and
    one for AIN input port (see :ref:`ad9434 user-guide`)

- (Optional) USB keyboard & mouse and a HDMI compatible monitor

More details as to why you need these, can be found at
:ref:`ad9434 prerequisites`.
...
In the following example, we will make a physical connection between the ADC
and an external signal generator, using SMA cables.
...
#. Configure ZedBoard for SD BOOT:
    ...
    .. image:: ../images/ad9434_zed_jtag_jumpers.jpg
        :width: 800
    ...
    project's README at :external+hdl:ref:`ad9434_fmc`.
...
#. Connect the :adi:`EVAL-AD9434` FMC board to the `ZedBoard <https://digilent.com/shop/zedboard-zynq-7000-arm-fpga-soc-development-board>`__
...
.. collapsible:: Complete boot log
   ...
   ::
      ...
      ** Unable to read file uEnv.txt **
      ...
      reading uImage
      8903144 bytes read in 513 ms (16.6 MiB/s)
      reading devicetree.dtb
      21489 bytes read in 28 ms (749 KiB/s)
      ** Unable to read file uramdisk.image.gz **
      ## Booting kernel from Legacy Image at 03000000 ...
         Image Name:   Linux-6.12.0-g298669e57d37
         Image Type:   ARM Linux Kernel Image (uncompressed)
         Data Size:    8903080 Bytes = 8.5 MiB
      ...
      ## Flattened Device Tree blob at 02a00000
         ...
         Loading Device Tree to 1eb12000, end 1eb1a3f0 ... OK

      Starting kernel ...

      Booting Linux on physical CPU 0x0
      Linux version 6.12.0-g298669e57d37 (root@ce3220224904) (arm-linux-gnueabi-gcc (Ubuntu 13.3.0-6ubuntu2~24.04) 13.3.0, GNU ld (GNU Binutils for Ubuntu) 2.42) #1 SMP PREEMPT Fri Mar 13 08:38:12 UTC 2026
      ...
      Memory: 359484K/524288K available (13312K kernel code, 907K rwdata, 10948K rodata, 1024K init, 496K bss, 32468K reserved, 131072K cma-reserved, 0K highmem)
      ...
      macb e000b000.ethernet eth0: Cadence GEM rev 0x00020118 at 0xe000b000 irq 41 (00:e0:22:02:be:d7)
      ...
      axi_sysid 45000000.sysid: [ad9434_fmc] on [zed] git branch <main> git <442ab0b2108743693b106422149d5c4988de2f4f> clean [2026-03-04 14:43:47] UTC
      ...
      -
      IDELAY 0x9
      cf_axi_adc 44a00000.axi-ad9434-core-lpc: ADI AIM (10.03.) probed ADC AD9434 as MASTER
      ...
      systemd[1]: kmod-static-nodes.service - Create List of Static Device Nodes was skipped because of an unmet condition check (ConditionFileNotEmpty=/lib/modules/6.12.0-g298669e57d37/modules.devname).
      ...
      systemd[1]: Starting systemd-tmpfiles-setup-dev-early.service - Create Static Device Nodes in /dev gracefully...
               Starting systemd-tmpfiles-setup-de… Devicsystemd[1]: systemd-tpm2-setup-early.service - Early TPM SRK Setup was skipped because of an unmet condition check (ConditionSecurity=measured-uki).
      ...
      Linux analog 6.12.0-g298669e57d37 #1 SMP PREEMPT Fri Mar 13 08:38:12 UTC 2026 armv7l
...
.. shell::
   ...
   iio:device2: axi-ad9434-core-lpc (buffer capable)
...
The :ref:`iio-oscilloscope` is a cross-platform application for interfacing
with IIO devices, enabling you to configure device parameters and visualize
data.
...
#. Press ``Refresh`` to display available IIO Devices and press ``Connect``.
    ...
    .. image:: ../images/ad9434_zed_osc_connection.png
        :width: 800
...
#. After the board is connected and a channel is enabled, hit the play button.
   Then the data capture window can be seen like in the shown picture.
    ...
    .. image:: ../images/ad9434_zed_osc_capture.png
        :width: 800
...
- no-OS project: :git-no-os:`projects/ad9434-fmc-500ebz`

Instructions on how to build the boot files from source can be found here:

- :external+no-OS:doc:`drivers/adc/ad9434`. More no-OS build
  details at :external+no-OS:doc:`build_guide`.
- :external+hdl:ref:`ad9434_fmc`. More HDL build details at
...
- AMD Xilinx `ZedBoard <https://digilent.com/shop/zedboard-zynq-7000-arm-fpga-soc-development-board>`__
...
.. image:: ../images/ad9434_zed_noos_setup.png
   :width: 800

Follow the steps in this order, to avoid damaging the components:

#. Connect the :adi:`EVAL-AD9434` FMC board to the `ZedBoard <https://digilent.com/shop/zedboard-zynq-7000-arm-fpga-soc-development-board>`__ **LPC**
   FMC socket
#. Configure `ZedBoard <https://digilent.com/shop/zedboard-zynq-7000-arm-fpga-soc-development-board>`__
   for SD card boot mode
...
.. collapsible:: Complete boot log
   ...
   ::
      ...
      AD9434 successfully initialized.
      ad9434_core: Successfully initialized (499871826 Hz)
      adc_delay: setting zero error delay (9)
      Capture done.
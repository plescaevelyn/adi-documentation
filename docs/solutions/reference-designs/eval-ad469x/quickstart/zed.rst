...
.. note::
   ...
   directory. Using the host PC, drag and drop the required files onto the BOOT
   partition, and use the EJECT function when removing the SD card from the
   reader.
...
- Instructions on how to manually build the boot files from source can be found
  ...
  - :external+hdl:ref:`ad469x_evb` build documentation. More HDL build details
...
.. image:: ../images/ad4696_zed_setup.jpg
   :align: center
   :width: 900
...
#. Get the
   ...
   .. important::
      ...
      To avoid potential issues, ensure the ZedBoard
      VADJ SELECT = 1.8 V.

      .. image:: ../images/zed_vadj.jpg
         :align: center
         :width: 200
...
#. Connect the :adi:`EVAL-AD4696FMCZ` to the FMC LPC connector.
...
#. The blue LD12/DONE LED and red LD0 LED should start blinking ~20-30 seconds
   ...
   .. tip::
      ...
      will instead boot with the BLUE LD12/DONE LED blinking immediately and
      LD7 blinking after ~20-30 seconds. This may indicate that an improved
      version of the ACE plugin is available if the SD card is updated to the
      latest version.
...
.. collapsible:: Complete boot log
   ...
   ::
      ...
      reading uImage
      8903584 bytes read in 516 ms (16.5 MiB/s)
      reading devicetree.dtb
      22553 bytes read in 30 ms (733.4 KiB/s)
      ** Unable to read file uramdisk.image.gz **
      ...
      ## Flattened Device Tree blob at 02a00000
         ...
         Loading Device Tree to 1eb11000, end 1eb19818 ... OK
      ...
      axi_sysid 45000000.sysid: [ad469x_evb] [SPI_4WIRE=0] on [zed] git branch <hdl_2026_r1> git <a77599a506f259d09aa871459427287a7cb92ebc> clean [2026-04-22 12:51:16] UTC
...
.. shell::
   ...
   iio:device0: ad4696 (buffer capable)
   iio:device1: xadc
...
:external+scopy:doc:`Scopy <index>` can be used to visualize the data captured
from the AD4696 evaluation board.

Connect the signal generator to one of the analog input channels on the
:adi:`EVAL-AD4696FMCZ` board using the appropriate connector.


.. image:: ../images/ad4696_zed_scopy.png
   :width: 1000
...
#. Press ``Refresh`` to display available IIO Devices and press
   ...
   .. image:: ../images/ad4696_zed_iio.png
      :width: 1000
...
#. After the board is connected and a channel is enabled, hit the play button.
   ...
   .. image:: ../images/ad4696_zed_iio_capture.png
      :width: 1000
...
- no-OS project: :git-no-os:`projects/ad469x_evb`

Instructions on how to build the boot files from source can be found below:

- :git-no-os:`projects/ad469x_evb`. More no-OS build details at
  :external+no-OS:doc:`build_guide`.

- :external+hdl:ref:`ad469x_evb`. More HDL build details at
...
- `ZedBoard <https://digilent.com/reference/programmable-logic/zedboard/start>`__
  Rev C or later
- :adi:`EVAL-AD4696FMCZ` FMC evaluation board
...
.. warning::
   ...
   Before executing below steps, **VADJ must be set to 1.8 V**.
   This can be done by changing VADJ jumper (JP18) to 1v8.

In the following example, we will prepare the evaluation setup by connecting
the AD469x evaluation board to the ZedBoard, configuring the board for JTAG
...
#. Configure ZedBoard for JTAG boot:
   ...
   .. image:: ../images/zed_baremetal_boot.jpg
      :width: 300

   The BOOT switches (JP7-JP11) and the MIO0 jumper (JP6) must be set for JTAG
   boot mode. VADJ must be set to 1.8 V, as specified in the HDL project's
   README at :external+hdl:ref:`ad469x_evb`. Optionally, to use USB peripheral
...
#. Connect your signal generator to the analog inputs of the
   ...
   connectors.
#. Plug the AD469x evaluation board into the FMC LPC Connector (J1)

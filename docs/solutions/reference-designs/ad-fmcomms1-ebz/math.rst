...
In a traditional radio transmit architecture, a baseband signal (voice or data),
is modulated to an intermediate frequency (IF) and then modulated again to the
transmit radio frequency (RF) and on the receive side, the opposite happened,
from RF to IF to baseband. This was done since inexpensive up-converting
modulators/down-converting demodulators only provide adequate performance over
roughly one decade of frequency conversion, so it is often necessary to execute
the complete up-conversion in two or more frequency hops.
...
Like most mathematical derivations, we will ignore the real world imperfections,
and these will be discussed at near the end.
...
.. math::
   ...
   x(t) = A \times (\frac{1}{2}e^{-j \kappa} + \frac{1}{2}e^{j \kappa}) \times
   (\frac{1}{2}e^{-j \omega} - \frac{1}{2}e^{j \omega}) +
             B \times (\frac{1}{2}e^{-j \kappa} - \frac{1}{2}e^{j \kappa})
             \times (\frac{1}{2}e^{-j \omega} + \frac{1}{2}e^{j \omega})

.. math::

   x(t) = \frac{A}{2} \times (e^{-j \kappa} + e^{j \kappa}) \times (e^{-j
   \omega} - e^{j \omega}) +
             \frac{B}{2} \times (e^{-j \kappa} - e^{j \kappa}) \times (e^{-j
             \omega} + e^{j \omega})
...
.. math::
   ...
   x(t) = \frac{1}{2} \times ((Ae^{-j \kappa -j \omega} + Ae^{j \kappa -j

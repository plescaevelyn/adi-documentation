ADIS16334 EVALUATION ON THE EVAL-ADIS
=====================================
...
For those who are on a tight timeline, connecting the :adi:`ADIS16334` to an embedded controller will provide the most flexibility in developing application firmware and will more closely reflect the final system design. The :adi:`ADIS16334/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16334/products/EVAL-ADIS16334/eb.html>` is the breakout board for the :adi:`ADIS16334` and may provide assistance in the process of hooking it up to an existing embedded processor system. For more information on the :adi:`ADIS16334/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16334/products/EVAL-ADIS16334/eb.html>` breakout boards, check out the following link: :adi:`ADIS16334/PCB Breakout Board Web Page <en/mems-sensors/mems-inertial-measurement-units/adis16334/products/EVAL-ADIS16334/eb.html>`.

EVAL-ADIS: PC EVALUATION
------------------------

For those who would prefer to perform PC-based evaluation of the :adi:`ADIS16334`, before developing their own embedded system, the :adi:`EVAL-ADIS` is the appropriate system to use. The remainder of this Wiki site will focus on PC-based evaluation with the :adi:`EVAL-ADIS` system.
...
The :adi:`ADIS16334/PCBZ <en/mems-sensors/mems-inertial-measurement-units/adis16334/products/EVAL-ADIS16334/eb.html>` includes one interface PCB, one flexible cable/connector and one :adi:`ADIS16334AMLZ <ADIS16334>` unit. Use the flexible cable/connector to install the :adi:`ADIS16334` onto the :adi:`EVAL-ADIS`, using the following five steps. Set interface PCB aside, as it is not used for connecting the :adi:`ADIS16334AMLZ <ADIS16334>` to the :adi:`EVAL-ADIS`.
...
NOTE: Most of the pictures in this section represent the :adi:`ADIS16448`, not the :adi:`ADIS16334`. The packages associated with these two products are close enough to illustrate the key points associated with installing the :adi:`ADIS16334`.
...
Slide the :adi:`ADIS16334AMLZ <ADIS16334>` body towards J4, while tucking the excess flexing under the connector interface, as show in the following pictures.
...
Secure the :adi:`ADIS16334AMLZ <ADIS16334>` body, using (4) M2x0.4x10mm machine screws (included with :adi:`EVAL-ADIS`) and the "D" mounting holes (EVAL-ADIS labels for mounting holes).
...
Set the jumper JP1 on the (:adi:`EVAL-ADIS`) to the **+5V** position setting for the :adi:`ADIS16334AMLZ <ADIS16334>`. The following picture shows JP1 in the **+3.3V** position
...
This section currently has no :adi:`ADIS16334`-specific content, but the :doc:`ADIS16448 Evaluation on the EVAL-ADIS Wiki Site </solutions/reference-designs/inertial-mems/imu/adis16448>` has some good examples to start with.
...
.. |image11| image:: ../images/eval-adis-nodut-334holes.png
   :width: 400

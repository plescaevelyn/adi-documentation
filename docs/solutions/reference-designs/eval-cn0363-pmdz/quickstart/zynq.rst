.. _eval-cn0363-pmdz-quickstart-zynq:

ZED Quick Start Guide
================================================================================
...
:adi:`EVAL-CN0363-PMDZ <CN0363>` on:

- `ZED Board <https://www.avnet.com/americas/products/avnet-boards/avnet-board-families/zedboard/>`_
...
- :external+hdl:ref:`cn0363` build documentation. More HDL build details at
...
- Avnet `ZED Board <https://www.avnet.com/americas/products/avnet-boards/avnet-board-families/zedboard/>`_
  FPGA board and its power supply
- :adi:`EVAL-CN0363-PMDZ <CN0363>` PMOD evaluation board
- SD card with at least 16GB of memory
- Micro-USB cable (UART serial console)
- Micro-USB OTG adapter and USB keyboard & mouse, with a
  HDMI compatible monitor (for graphical interface)
- LAN cable (Ethernet)

More details as to why you need these can be found at
:ref:`eval-cn0363-pmdz-prerequisites`.
...
#. Connect the :adi:`EVAL-CN0363-PMDZ <CN0363>` PMOD board to the
   ZED board **PMOD JA** connector
...
#. Connect to the ZED board using one or both of the following:

   - **Serial console (UART):** Connect a Micro-USB cable to the USB UART
     port (J14). Install jumper JP3.
   - **Graphical interface (OTG):** Connect a USB keyboard and mouse via
     the Micro-USB OTG port (J13) using an OTG adapter. Install jumper JP2.

   .. image:: ../images/zed_cn0363_otg_jumpers.jpg
      :width: 400
#. Plug-in 12 V power supply to the ZED board (J20)
#. Plug-in an Ethernet cable from your router/switch to the Ethernet port on
   the ZED board
#. Connect a monitor to the ZED board by HDMI (J9)
#. Configure the `ZED board <https://www.avnet.com/americas/products/avnet-boards/avnet-board-families/zedboard/>`_
   for SD card boot mode switches and the 3.3v VADJ jumper
   (as seen in the below picture)

   .. image:: ../images/zed_cn0363_jumpers.jpg
      :width: 400
#. Insert SD card into the SD card socket on the ZED board (J12)
#. Plug-in a 6V to 12V power supply connected to the EVAL-CN0363-PMDZ
#. Turn on the power switch on the ZED board (SW8)
#. Observe Kernel and serial console output messages on your terminal (use
   the first ttyUSB or COM port registered)

Below is a simplified schematic of the hardware setup:
...
.. note::
   ...
   the *ZedBoard Hardware User Guide*, available on the
   `ZedBoard doc page <https://www.avnet.com/americas/products/avnet-boards/avnet-board-families/zedboard/>`_,
...
.. collapsible:: Complete boot log
   ...
   ::
      ...
      21191 bytes read in 30 ms (689.5 KiB/s)
      ** Unable to read file uramdisk.image.gz **
      ...
      ## Flattened Device Tree blob at 02a00000
         ...
         Loading Device Tree to 1eb12000, end 1eb1a2c6 ... OK
      ...
      ad_dpot spi0.1: ad5201 32-Position Digital Potentiometer registered
      ...
      macb e000b000.ethernet eth0: Cadence GEM rev 0x00020118 at 0xe000b000 irq 44 (00:0a:35:00:01:28)
      ...
      ad7173 spi0.0: supply vref not found, using dummy regulator
      ad7173 spi0.0: supply vref2 not found, using dummy regulator
      ad7173 spi0.0: supply avdd not found, using dummy regulator
      ...
      axi_sysid 45000000.axi-sysid-0: [cn0363] on [zed] git branch <hdl_2023_r2> git <2156ac7e874a1dc321d9f64a325009fafe563419> clean [2024-11-01 16:10:30] UTC
...
Turn on the main power switch on the ZED board (SW8). After a few seconds the
blue DONE LED (LD12) will turn on. It takes a while for the system to
be completely booted, after around 30 seconds you will see the following
screen on the HDMI monitor.

To start the EVAL-CN0363-PMDZ colorimeter application go to the top-left
**"Applications Menu"** and to the **"Other"** section and click on
**"ADI CN0363 Colorimeter"**.

.. image:: ../images/colorimeter_open_app.png
   :align: center
   :width: 800px

Now you should see the following screen and you can use the EVAL-CN0363-PMDZ
to perform measurements.

.. image:: ../images/colorimeter_app_startup.png
   :align: center
   :width: 800px

For more information on how to use the application see the
:doc:`EVAL-CN0363-PMDZ Colorimeter Application User Guide <../software/linux/colorimeter>`.

...
- Python 3.11 or newer (3.10–3.14 tested with pyadi-iio). Ensure that a
  compatible version is installed on your system before continuing.

- Git command-line tools installed.

- Raspberry Pi with :adi:`kuiper-linux <kuiper>` image installed. Follow the
  instructions in the :external+kuiper:doc:`index` documentation to prepare the
  Raspberry Pi.

- Ensure that the first two **Software Setup** steps described in
  :ref:`software-setup` (*Building and Flashing the Micro-SD Card* and
  *Configuring the Micro-SD Card*) are performed **with the Kuiper Linux image**,
  not with previous Kuiper releases. These steps must be redone after flashing
  Kuiper Linux to ensure proper compatibility before continuing.
...
- 1x Raspberry Pi 4 Model B or newer running Kuiper 2
...
- 1x 5V, 1A Power Supply for the Power Resistor (a 5V USB wall adapter works well)
...
4. Attach the power resistor to the TMP01 sensor, making sure they are in good thermal contact. 
   You can use cyanoacrylate glue (permanent), silicone adhesive, or thermal transfer compound and 
   a rubber band to hold the two together. The power resistor acts as a heat source simulating a GPU 
   under load (similar to running a graphics-intensive application like `SuperTuxKart <https://supertuxkart.net/Main_Page.html>`__).
...
7. Apply 5V across the 6.2Ω, 10W Power Resistor. The resistor will dissipate
   approximately 4W and may get quite warm, so use caution.
...
1. Clone the repository:
   ...
   .. shell::
      ...
      $ cd pyadi-iio
      $ git checkout release/ad-swiot1l-sl
...
1. Use the firmware images provided below. These images configure the boards with
   static IP addresses:
...
2. Follow the official update instructions here:
   :ref:`updating-the-ad-swiot1l-sl-firmware`.

3. Repeat the process for **both boards**.
...
Open a terminal and edit ``/boot/config.txt`` using Nano (or your favorite text
editor):
...
5. Verify routing and connectivity to the boards:
   ...
   .. figure:: ad-rpi-t1lpse-sl-sample-application-ping.png
      :align: center
      :width: 500

      Verifying connectivity to the AD-SWIOT1L-SL boards
...
Run the Temperature Controller example, specifying board IP addresses and
thresholds:

...
proceed to the `Hardware Setup <https://wiki.analog.com/testing>`_ section.

Obtain the pre-built SD card Zynq image for ZC706 pre-installed with the
fmcomms11 test program from this `link <https://wiki.analog.com/testing>`_. If
...
about how to use the Win32 disk manager. Etcher, on the other hand, is easier
to use than win32 disk manager. Example Disk writer software:
...
-  Attach the FMCOMMS11 that has a hardware loopback to the Carrier board via
   the FMC HPC connector (J37).
...
-  In case of Program errors, the message box like the image below will appear:
   ...
   board that was tested before that "passed" to check the state of the carrier
   board.

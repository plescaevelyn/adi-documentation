.. _ad_fmcjesdadc1_ebz microblaze_quickstart:
...
.. warning::
   ...
   Support for the AD-FMCJESDADC1-EBZ was discontinued starting with the
   **2022_R2** Kuiper Linux release and it won't be supported in future
   releases. The last release with pre-built files is **2022_r2** - see all
   releases at :git-kuiper:`releases+`.
   The project source code can still be found on the
...
- Connect the AD-FMCJESDADC1-EBZ FMC board to the FPGA carrier:
  on the KC705: FMC HPC, or on the VC707: FMC1 HPC connector.
...
Download the pre-built image for your device from the last supported release
(2022_r2) at :git-kuiper:`releases+`.

There are two ways of loading the design. One is using the XMD command line.
Open an XMD command window/shell and enter the commands manually.

Below is an example and the file names may vary.

.. collapsible:: XMD shell
...
The second method is to run the TCL script which takes care of loading the
bitfile and the Linux image. Run Vivado TCL Shell from Windows Start menu ->
Xilinx Design Suite -> Vivado -> Vivado TCL Shell. (In Linux, source the
settings.sh file first.)

Then run the TCL script:

.. collapsible:: TCL shell
...
If your FPGA carrier board (:xilinx:`KC705`, :xilinx:`VC707`) features an LCD
display and is connected to a DHCP-enabled network, you should also see the IP
address printed on the display. This allows remote connection to the board
(SSH, libiio remote).

If the IP address shows as 192.168.2.2, DHCP has failed and the board is
using its default address, which may not be within your subnet.

.. figure:: ../images/ml605-lcd.png
   :alt: LCD image showing board IP address
   :width: 400

   FPGA carrier LCD displaying the board IP address
...
.. collapsible:: Complete boot log
   ...
   ::
      ...
      Linux version 3.17.0-126736-gdf1ca8f-dirty (michael@mhenneri-D04) (gcc version 4.8.3 20140131 (prerelease) (crosstool-NG 1.18.0) ) #1943 Wed Dec 17 11:03:18 CET 2014
      ...
      NR_IRQS:128
      /axi@0/axi-intc@41200000: num_irq=21, edge=0x3810
      /axi@0/axi-timer@41c00000: irq=1
      xilinx_timer_set_mode: shutdown
      xilinx_timer_set_mode: periodic
      sched_clock: 32 bits at 99MHz, resolution 10ns, wraps every 43383508981ns
      ...
      xilinx_spi 44a70000.axi-quad-spi: at 0x44A70000 mapped to 0xb0160000, irq=13
      libphy: Fixed MDIO Bus: probed
      xilinx_emaclite 40e00000.network: Device Tree Probing
      libphy: Xilinx Emaclite MDIO: probed
      xilinx_emaclite 40e00000.network: MAC address is now 00:0a:35:00:00:10
      ...
      platform 44a10000.axi-ad9250-hpc-0: Driver cf_axi_adc requests probe deferral
      platform 44a20000.axi-ad9250-hpc-1: Driver cf_axi_adc requests probe deferral
      spi spi32765.2: Driver ad9467 requests probe deferral
      spi spi32765.3: Driver ad9467 requests probe deferral
      platform 44a91000.axi-jesd204b-rx: Driver cf_axi_jesd204b_v51 requests probe deferral
      platform 44a60000.axi-jesd-gt-rx-tx: Driver cf_axi_jesd204b_gt requests probe deferral
      ad9517 spi32765.1: probed
      ...
      spi spi32765.3: Driver ad9467 requests probe deferral
      platform 44a91000.axi-jesd204b-rx: Driver cf_axi_jesd204b_v51 requests probe deferral
      cf_axi_jesd204b_gt 44a60000.axi-jesd-gt-rx-tx: AXI-JESD204B (6.00.b) at 0x44A60000 mapped to 0xb01a0000,
      platform 44a10000.axi-ad9250-hpc-0: Driver cf_axi_adc requests probe deferral
      platform 44a20000.axi-ad9250-hpc-1: Driver cf_axi_adc requests probe deferral
      spi spi32765.2: Driver ad9467 requests probe deferral
      spi spi32765.3: Driver ad9467 requests probe deferral
      ...
      platform 44a10000.axi-ad9250-hpc-0: Driver cf_axi_adc requests probe deferral
      platform 44a20000.axi-ad9250-hpc-1: Driver cf_axi_adc requests probe deferral
      ad9467 spi32765.2: AD9250 PLL LOCKED, JESD204B Link Ready
      ad9467 spi32765.3: AD9250 PLL LOCKED, JESD204B Link Ready
      cf_axi_adc 44a10000.axi-ad9250-hpc-0: ADI AIM (8.00.b) at 0x44A10000 mapped to 0xb01e0000, probed ADC AD9250 as MASTER
      cf_axi_adc 44a20000.axi-ad9250-hpc-1: ADI AIM (8.00.b) at 0x44A20000 mapped to 0xb0200000, probed ADC AD9250 as MASTER
      ...
      Sending discover...
      Sending select for 10.44.2.153...
      Lease of 10.44.2.153 obtained, lease time 86400
      deleting routers
